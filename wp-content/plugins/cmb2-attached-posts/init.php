...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		if ( $this->field->options( 'filter_boxes' ) ) {
			$filter_boxes = '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( 'Filtrar %s', 'cmb' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>';
		}
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( '%s vinculados', 'cmb' ), $post_type_labels ) . '</h4>';
